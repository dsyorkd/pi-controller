...
func (rl *GPIORateLimiter) RateLimit() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if rl.isWhitelisted(c.ClientIP()) {
			rl.logger.WithField("client_ip", c.ClientIP()).Debug("Whitelisted IP, skipping rate limit")
			c.Next()
			return
		}
...
func (rl *GPIORateLimiter) getLimiter(clientID string) *rate.Limiter {
	...
	rl.logger.WithField("client_id", clientID).Debug("Created new GPIO rate limiter for client")
